...
module.exports = {
    ...
    CheckTCPSelf: function(xip, wwwport) {
        //Check if this node is responding on external IP
        return new Promise(function(resolve,reject){
            var objTCPOptions = new Object();
            objTCPOptions.address = xip;
            objTCPOptions.port = parseInt(wwwport);
            ...
            tcpping.ping(objTCPOptions, function(err, data) {
                ...
                if (!err) {
                    ...
                    if(!isNaN(data.avg)) {
                        ...
                        logger.info("Self TCP check: Success against "+xip+":"+wwwport);
                    }
                    else {
                        bAlive = false;
                        logger.warn("Self TCP check failed against "+xip+":"+wwwport +".  Need firewall config?  Setting wwwport to 0 (not used).");
